...
Every bug fix, feature update, and meaningful project change should be recorded in `CHANGELOG.md`. Keep entries short: use one-line bullets such as `Improved wall snapping accuracy` or `Small improvement to visual QA screenshots`. Avoid long paragraphs; keep verification notes to one compact line when useful. These project versions are separate from OpenPlanTrace's JSON contract schema versions such as `openplantrace.scan.v62`.
