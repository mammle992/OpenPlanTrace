...
internal static class BatchScanComparisonJsonSchema
{
    public const string CurrentSchemaVersion = BatchScanComparisonResult.CurrentSchemaVersion;

    public const string CurrentResourceName =
        "OpenPlanTrace.Cli.Schemas.openplantrace.batch-comparison.v1.schema.json";
    ...
    {
        var assembly = typeof(BatchScanComparisonJsonSchema).GetTypeInfo().Assembly;
        return assembly.GetManifestResourceStream(CurrentResourceName)
            ?? throw new InvalidOperationException(
                $"Embedded OpenPlanTrace batch comparison schema resource '{CurrentResourceName}' was not found.");
    }
}
