...
public static class BatchScanManifestJsonSchema
{
    public const string CurrentSchemaVersion = BatchScanManifest.CurrentSchemaVersion;

    public const string CurrentResourceName =
        "OpenPlanTrace.Schemas.openplantrace.batch-manifest.v1.schema.json";
    ...
    {
        var assembly = typeof(BatchScanManifestJsonSchema).GetTypeInfo().Assembly;
        return assembly.GetManifestResourceStream(CurrentResourceName)
            ?? throw new InvalidOperationException(
                $"Embedded OpenPlanTrace batch manifest schema resource '{CurrentResourceName}' was not found.");
    }
}
