...
public static class BenchmarkComparisonJsonSchema
{
    public const string CurrentSchemaVersion = BenchmarkComparisonResult.CurrentSchemaVersion;

    public const string CurrentResourceName =
        "OpenPlanTrace.Schemas.openplantrace.benchmark-comparison.v1.schema.json";
    ...
    {
        var assembly = typeof(BenchmarkComparisonJsonSchema).GetTypeInfo().Assembly;
        return assembly.GetManifestResourceStream(CurrentResourceName)
            ?? throw new InvalidOperationException(
                $"Embedded OpenPlanTrace benchmark comparison schema resource '{CurrentResourceName}' was not found.");
    }
}
