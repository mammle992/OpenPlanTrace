...
public static class BenchmarkRunResultJsonSchema
{
    public const string CurrentSchemaVersion = BenchmarkRunResult.CurrentSchemaVersion;

    public const string CurrentResourceName =
        "OpenPlanTrace.Schemas.openplantrace.benchmark-result.v1.schema.json";
    ...
    {
        var assembly = typeof(BenchmarkRunResultJsonSchema).GetTypeInfo().Assembly;
        return assembly.GetManifestResourceStream(CurrentResourceName)
            ?? throw new InvalidOperationException(
                $"Embedded OpenPlanTrace benchmark result schema resource '{CurrentResourceName}' was not found.");
    }
}
